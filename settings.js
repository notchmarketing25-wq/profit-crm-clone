...
class SettingsManager {
    ...
    updateColors() {
        ...
        const gradientElements = document.querySelectorAll('.login-page, .dashboard-header, .group-icon, .card-icon');
    ...
    updateLogoPreview(logoUrl) {
        const previewElements = document.querySelectorAll('.preview-logo, .dashboard-logo');
        previewElements.forEach(el => {
            if (logoUrl) {
                el.innerHTML = `<img src="${logoUrl}" alt="Logo" style="height: 40px; width: auto;">`;
            }
        });
    }
    ...
    restoreFromBackup(backupData) {
        if (confirm('هل أنت متأكد من استعادة النسخة الاحتياطية؟ سيتم استبدال الإعدادات الحالية.')) {
            this.settings = { ...this.settings, ...backupData };
            this.saveSettings();
            this.populateForm();
    ...
    bindFormSubmissions() {
        ...
        window.saveAutoBackupSettings = () => {
            ...
            if (frequency) {
                ...
                this.saveSettings();
                showNotification('تم حفظ إعدادات النسخ التلقائي', 'success');
            }
        };
